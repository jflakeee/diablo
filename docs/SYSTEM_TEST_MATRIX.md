...
| 동료 장비 | 무기·방어구 장착, 전투 능력치 반영, 파손 제외, 저장·복원 | `[SYSTEM]` 동료 슬롯/스케일링 검사, `[SAVE] checks=14` | PASS |
...
| 장비 요구조건 | 모든 베이스의 레벨·힘·민첩 조건, 자동/수동·동료 장착 차단, UI 사유 표시 | `[SYSTEM]` 경계·레거시 검사, `[COVERAGE]` 전 베이스 필드 검사 | PASS |
| 양손 반지 | 좌·우 반지 독립 장착, 빈 슬롯 우선·낮은 점수 교체, 옵션 중첩 | `[SYSTEM]` 생명·마나·화염·냉기 옵션 합산 검사 | PASS |
| 저장 | 원자 교체, 왕복, v1→v9 이관, 단일 반지→좌·우 슬롯 및 전체 진행 상태, 손상 거부·백업 복구·중단 격리 | `[SAVE] checks=14` | PASS |
