...
- [x] 런타임 자산 출처 목록상 외부 게임 추출 이미지·음원·폰트·코드 0개
- [x] 모든 컴파일 자산에 recipe ID/type, generator version, 입력 해시와 출력 MD5 존재
- [x] 참조 시스템별 독립 테스트 명세와 자동 검증 행렬 존재 (`docs/SYSTEM_TEST_MATRIX.md`)
- [x] 프로젝트명·클래스·스킬·몬스터·고유 장비·각인 재료/조합에 독자 ID와 표시명 사용 및 금지명 회귀검사
- [x] 모바일 주요 조작 hit area 48 logical px 이상
- [x] 주 공격 hit area 72 logical px 이상
- [x] 4:3~20:9에서 전투 HUD 겹침 없음(4개 기준 화면 자동 검사)
- [x] OS safe area를 논리 좌표로 변환하고 16px 내부 배치
- [x] UI 배율 80/100/120/140% 및 보통/큰 텍스트 모드와 영속 설정 제공
- [x] 색상 외에 글자·문양·수량으로 전투 조작 상태 구분
- [x] 저사양 기준 25Hz 논리틱과 기존 렌더 목표 유지
- [x] strict 릴리스 검증에서 필수 디자인 자산 폴백 0
- [ ] 상업 배포 전 독립 IP 검토 완료
