...
func _ready() -> void:
	...
	print("[P3] ready — life=%d dmg=%d-%d def=%d MF=%d" % [
...
func _process(delta: float) -> void:
	...
	if _auto_quit and elapsed >= 12.0:
		print("[P3][RESULT] kills=%d drops=%d picked=%d bag=%d" % [_kills, _items_dropped, _items_picked, _inventory.size()])
		print("[P3][RESULT] weapon=%s dmg=%d-%d def=%d" % [wn, _player.dmg_min, _player.dmg_max, _player.defense])
		var equipped_any: bool = (not _equipped["weapon"].is_empty()) or (not _equipped["armor"].is_empty())
		var ok: bool = _items_dropped > 0 and _items_picked > 0 and equipped_any
		print("[P3][RESULT] verdict=", ("PASS" if ok else "INCOMPLETE"))
		get_tree().quit()
...
func _recompute_player() -> void:
	...
	for slot in ["weapon", "armor"]:
		...
		for stat in it["affixes"]:
			eq[stat] = int(eq.get(stat, 0)) + int(it["affixes"][stat])
	_eq = eq
