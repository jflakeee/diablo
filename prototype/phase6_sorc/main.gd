...
func _ready() -> void:
	...
	# 소서리스 파생(Part 1 §2): Life 40+2·Vit+1·Lv / Mana 35+2·En+2·Lv
	_player.max_life = 40 + 2 * _player.stat_vit + _player.level
	_player.base_max_life = _player.max_life
	_player.life = _player.max_life
	_player.max_mana = 35 + 2 * _player.stat_energy + 2 * _player.level
	...
	_spawn_monster("Fallen", Color(0.2, 0.35, 0.85), 20, 28, 2, 15, 55, 8, 1, 3, 3.4, 14, 6)
	_spawn_monster("Fallen", Color(0.2, 0.35, 0.85), 20, 28, 2, 15, 55, 8, 1, 3, 3.4, 6, 14)
	...
	print("[P6] ready — class=%s life=%d mana=%d def=%d skills=fireball/static/teleport" % [
		_class, _player.max_life, _player.max_mana, _player.defense])
...
func _process(delta: float) -> void:
	...
	if _auto_quit and elapsed >= 12.0:
		print("[P6][RESULT] class=%s level=%d kills=%d spells_cast=%d spell_hits=%d life=%d/%d" % [
			_class, _player.level, _kills, _spells_cast, _spell_hits, _player.life, _player.max_life])
		var ok: bool = _spells_cast > 0 and _spell_hits > 0 and _kills > 0
		print("[P6][RESULT] verdict=", ("PASS" if ok else "INCOMPLETE"))
		get_tree().quit()
