...
func _ready() -> void:
	...
	_spawn_monster("Fallen", Color(0.2, 0.35, 0.85), 20, 28, 2, 15, 55, 8, 1, 3, 3.8, 6, 3)
	var sf1 := _spawn_monster("Spike Fiend", Color(0.8, 0.5, 0.2), 18, 20, 3, 18, 60, 10, 1, 4, 3.0, 15, 5)
	sf1.set_meta("kind", "ranged")
	var sf2 := _spawn_monster("Spike Fiend", Color(0.8, 0.5, 0.2), 18, 20, 3, 18, 60, 10, 1, 4, 3.0, 4, 15)
	sf2.set_meta("kind", "ranged")
	_spawn_monster("Zombie", Color(0.45, 0.55, 0.25), 24, 32, 4, 40, 80, 40, 2, 5, 2.4, 13, 12)
	# 액트1 보스: Andariel — 3패턴 공격 루틴(근접/독노바/독분사). 프로토타입 HP 축소(정식 Normal 1024, Part 3 §4)
	...
	print("[P5] ready — life=%d dmg=%d-%d def=%d boss=%s(hp %d)" % [
		_player.max_life, _player.dmg_min, _player.dmg_max, _player.defense, _boss.actor_name, _boss.max_life])
...
func _process(delta: float) -> void:
	...
	if _auto_quit and elapsed >= 14.0:
		var bhp := _boss.life if is_instance_valid(_boss) else 0
		print("[P5][RESULT] boss_hp=%d/%d melee=%d nova=%d spray=%d kills=%d" % [
			bhp, _boss.max_life, _boss_melee_cnt, _boss_nova_cnt, _boss_spray_cnt, _kills])
		var ok: bool = _boss != null and _boss_nova_cnt > 0 and _boss_spray_cnt > 0
		print("[P5][RESULT] verdict=", ("PASS" if ok else "INCOMPLETE"))
		get_tree().quit()
