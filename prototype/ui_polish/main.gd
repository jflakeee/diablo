...
func _ready() -> void:
	...
	_ui_selftest()
	print("[UI] ready — class=%s life=%d def=%d (Bag/Char 패널, 드래그&드롭 장착)" % [
		_class, _player.max_life, _player.defense])
...
func _process(delta: float) -> void:
	...
	if _auto_quit and elapsed >= 12.0:
		print("[UI][RESULT] class=%s level=%d kills=%d bag=%d life=%d/%d (UI selftest above)" % [
			_class, _player.level, _kills, _inventory.size(), _player.life, _player.max_life])
		var ok: bool = _kills > 0 and _player.alive
		print("[UI][RESULT] game verdict=", ("PASS" if ok else "PLAYER_DIED"))
		get_tree().quit()
