...
func _ready() -> void:
	...
	# 몬스터를 data/monsters.json에서 로드(data-driven)
	for md in Data.monsters():
		var col := Color(float(md["color"][0]), float(md["color"][1]), float(md["color"][2]))
		var m := _spawn_monster(String(md["name"]), col, int(md["w"]), int(md["h"]), int(md["level"]), int(md["hp"]), int(md["ar"]), int(md["def"]), int(md["dmin"]), int(md["dmax"]), float(md["speed"]), int(md["gx"]), int(md["gy"]))
	...
	_data_selftest()
	print("[DD] ready — class=%s life=%d mana=%d def=%d boss=%s(hp %d)" % [
		_class, _player.max_life, _player.max_mana, _player.defense, _boss.actor_name, _boss.max_life])
...
func _process(delta: float) -> void:
	...
	if _auto_quit and elapsed >= 15.0:
		var bhp := _boss.life if is_instance_valid(_boss) else -1
		print("[DD][RESULT] class=%s level=%d kills=%d boss_hp=%d/%d life=%d/%d (monsters from JSON)" % [
			_class, _player.level, _kills, bhp, _boss.max_life, _player.life, _player.max_life])
		var ok: bool = _kills > 0 and _player.alive
		print("[DD][RESULT] verdict=", ("PASS" if ok else "PLAYER_DIED"))
		get_tree().quit()
