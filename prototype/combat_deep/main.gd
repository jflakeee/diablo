...
func _ready() -> void:
	...
	_combat_deep_selftest()
	print("[CD] ready — class=%s life=%d def=%d block=%d leech=%d" % [
		_class, _player.max_life, _player.defense, _player.block_val, _player.leech_pct])
...
func _process(delta: float) -> void:
	...
	if _auto_quit and elapsed >= 12.0:
		print("[CD][RESULT] class=%s level=%d kills=%d life=%d/%d (combat_deep selftest above)" % [
			_class, _player.level, _kills, _player.life, _player.max_life])
		var ok: bool = _kills > 0 and _player.alive
		print("[CD][RESULT] game verdict=", ("PASS" if ok else "PLAYER_DIED"))
		get_tree().quit()
