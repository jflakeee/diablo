...
const Save := preload("res://save.gd")
...
func _ready() -> void:
	...
	boss.res_fire = -50   # 안다리엘 화염 약점 (Part 3 §4)
	_boss = boss
	...
	_save_selftest()
	print("[SV] ready — class=%s life=%d (S=저장, L=불러오기)" % [_class, _player.max_life])
...
func _gather_state() -> Dictionary:
	return {
		...
		"skills": _player.skills.duplicate(true),
		"inventory": _inventory.duplicate(true),
		"equipped": {
			"weapon": (_equipped["weapon"] as Dictionary).duplicate(true),
			"armor": (_equipped["armor"] as Dictionary).duplicate(true),
		},
		"kills": _kills,
	}

func _apply_state(s: Dictionary) -> void:
	_class = String(s.get("class", _class))
	_player.level = int(s.get("level", 1))
	_player.xp = int(s.get("xp", 0))
	_stat_points = int(s.get("stat_points", 0))
	_player.skill_points = int(s.get("skill_points", 0))
	_player.stat_str = int(s.get("stat_str", 10))
	_player.stat_dex = int(s.get("stat_dex", 10))
	_player.stat_vit = int(s.get("stat_vit", 10))
	_player.stat_energy = int(s.get("stat_energy", 10))
	var sk: Dictionary = s.get("skills", {})
	_player.skills = sk.duplicate(true)
	var inv: Array = s.get("inventory", [])
	_inventory = inv.duplicate(true)
	var eq: Dictionary = s.get("equipped", {})
	_equipped = {
		"weapon": (eq.get("weapon", {}) as Dictionary).duplicate(true),
		"armor": (eq.get("armor", {}) as Dictionary).duplicate(true),
	}
	_kills = int(s.get("kills", 0))
	_recompute_player()
	_rebuild_inv()
	_rebuild_char()
	_combat_log = "게임 불러옴 (Lv %d)" % _player.level

func _save_game() -> void:
	if Save.save_state(_gather_state()):
		_combat_log = "저장 완료"
...
func _save_selftest() -> void:
	# 상태 조작 → 저장 → 로드 → 라운드트립 검증
	_player.level = 7
	_player.stat_str = 44
	_player.skills["bash"] = 5
	_inventory.append(Item.generate(_rng, Item.WEAPON_BASES[2], 20, "rare"))
	_kills = 12
	var before := _gather_state()
	Save.save_state(before)
	var loaded := Save.load_state()
	var sk: Dictionary = loaded.get("skills", {})
	var inv: Array = loaded.get("inventory", [])
	var t1: bool = int(loaded.get("level", 0)) == 7
	var t2: bool = int(loaded.get("stat_str", 0)) == 44
	var t3: bool = int(sk.get("bash", 0)) == 5
	var t4: bool = inv.size() == (before["inventory"] as Array).size()
	var t5: bool = int(loaded.get("kills", 0)) == 12
	print("[SV] save→load: level=%d str=%d bash=%d inv=%d kills=%d" % [
		int(loaded.get("level", 0)), int(loaded.get("stat_str", 0)), int(sk.get("bash", 0)), inv.size(), int(loaded.get("kills", 0))])
	# 적용 라운드트립: 플레이어 변조 후 로드 복원
	_player.level = 1
	_player.stat_str = 1
	_apply_state(loaded)
	var t6: bool = _player.level == 7 and _player.stat_str == 44 and _player.skill_level("bash") == 5
	print("[SV] apply: 변조후 로드 → level=%d str=%d bash=%d : %s" % [
		_player.level, _player.stat_str, _player.skill_level("bash"), str(t6)])
	print("[SV][RESULT] save_selftest verdict=", ("PASS" if (t1 and t2 and t3 and t4 and t5 and t6) else "FAIL"))
...
func _process(delta: float) -> void:
	...
	if _auto_quit and elapsed >= 12.0:
		# 게임 중 저장→로드 라운드트립도 확인
		_save_game()
		var reloaded := Save.load_state()
		var save_ok: bool = int(reloaded.get("level", -1)) == _player.level and int(reloaded.get("kills", -1)) == _kills
		print("[SV][RESULT] class=%s level=%d kills=%d life=%d/%d save_roundtrip=%s" % [
			_class, _player.level, _kills, _player.life, _player.max_life, str(save_ok)])
		var ok: bool = _kills > 0 and _player.alive and save_ok
		print("[SV][RESULT] game verdict=", ("PASS" if ok else "FAIL"))
		get_tree().quit()
